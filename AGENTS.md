...
- 問題の修正はハック的なワークアラウンドではなく、根本原因に対処する。小手先の対応はコードの見通しを悪くしてメンテナンス性を損ない、複雑な挙動によってUXを悪化させるリスクがある。ライブラリが提供する拡張ポイント（オプション、コールバック等）があるなら、CSSや外部からの上書きで症状を隠すのではなく、それを活用して本質的に正しい実装をする（参考: ADR-0004）
- 変更後は `npm run format` でコードを整形し、`npm run lint` と `npm test` と `npm run test:e2e` で品質を確認する

### やってはいけないこと

- **CommonJSの使用**: `require()` / `module.exports` は使用禁止。すべてのコードはTypeScriptのESモジュール（`import` / `export`）で記述する。サーバーサイドのコードは `tsx` で実行する
...
- **その場限りのコメント**: 特定のIssueやPull-Request、作業にのみ固有のコメントを書かない（例: 「// この行を追加」のようなコメントは将来見返したときに意味がわからない）
- **説明用ファイルのコミット**: IssueやPR内での説明や、ユーザーとのやりとりのために必要となる説明ファイルや画像などをリポジトリにコミットしない。添付が必要な場合は、IssueやPRに直接添付する

### ワークフロー上のルール

- **課題管理**: 課題はGitHub Issuesで管理する。AIで作った課題には`created-by-ai`ラベルを付与する
- **コミット前の品質チェック**: コミットする前に、`npm run format` でコードを整形し、`npm run lint` と `npm test` と `npm run test:e2e` を実行する。余分なdiffやバグの混入を避けるため
- **小さく独立したコミット**: コミットの単位は互いに独立した小さな単位に分ける。複数の作業や、リファクタリングと変更などの性質が異なる作業は別々のコミットにする。あとから履歴を追いかけやすくするため
...
ADRは人間の判断を示した「判例」のようなものであり、AIエージェントはこの判断に沿って行動すること。
ただし、ADRが前提とした条件や制約が変わった場合は、当時の判断と反する提案をしてもよい。
ADRを作成するタイミングや内容は人間が決定する。AIエージェントは、それに従ってADRを作成すること。自己判断で作成してはならない。
...
- **ID生成**: ランダムな英数字12文字（`lib/id.ts`）。ソート順は `created_at` カラムで管理
