...
## 重要：ADRはユーザーの指示があるときのみ作成する

**ADRは人間がどのように判断したかを記録する「判例」である。LLMの判断や作業を記録するものではない。**

このスキルを使用するのは、**ユーザーから明示的にADR作成の指示があった場合のみ**である：

- ✅ 良い例：ユーザーが「この機能を実装しないと決めたので、ADRを作成してください」と指示した場合
- ❌ 悪い例：LLMが作業中に「この判断は重要だからADRを作成しよう」と自律的に判断した場合

LLMがユーザーに代わって判断を記録することはできない。ADRはあくまで**人間の判断を代筆する**ツールとして使用すること。
