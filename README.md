...
| `YPERSISTENCE`        | Yjsドキュメントを永続化するディレクトリパス。設定しない場合はメモリのみ（再起動で消失）   | 未設定（メモリのみ）    |
| `API_URL`             | クリーンアップスクリプト（`scripts/cleanup-archives.js`）で使用するAPIサーバーのベースURL | `http://localhost:3000` |

詳細は `.env.example` ファイルを参照してください。

**注意**: サーバー再起動時にページ内容が消えないようにするため、`YPERSISTENCE` 環境変数を設定することを推奨します。npm scriptsではデフォルトで `.yjs` ディレクトリに永続化されます。
