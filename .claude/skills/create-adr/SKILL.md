---
name: create-adr
description: ユーザーからの指示に従ってArchitecture Decision Record (ADR) を作成するスキル。技術的・機能的な重要な判断を記録し、将来の開発における判断基準を明確化する。
---

# ADR作成スキル

このスキルは、Architecture Decision Record (ADR) を作成するためのワークフローです。ADRは、人間が下した重要な技術的・機能的判断を記録した「判例」であり、以下の目的を持ちます：
...
この状況に該当するかどうかの判断は人間が行います。
AIエージェントはそれに従ってADRを作成してください。
...
これらの内容についてのADR作成を求められた場合は、不要である旨を説明し、それでも作成するかどうかの判断を仰いでください。
...
- 関連する/対応しないGitHub Issueがあれば番号を記載
- 実装する/しないコードの範囲
- 将来の再検討が必要になる条件
- 影響を受けるADRがあればその番号を記載
...
確認後、Prettierでフォーマットを整えて、EditorConfigに準拠しているかチェックしてください：
