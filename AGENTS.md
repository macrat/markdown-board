...
**ADRは人間がどのように判断したかを記録する「判例」である。LLMの判断や作業を記録するものではない。**

- ADRは、プロジェクトオーナーや開発者といった**人間が下した重要な判断**を記録するためのもの
- LLMが自身の判断でADRを作成してはならない
- LLMはユーザーからの**明示的な指示を受けてADRを代筆する**ことはあっても、自律的にADRを作成してはならない
- 例: ユーザーが「この機能を実装しないと決めたので、ADRを作成してください」と指示した場合にのみ、LLMはADRを作成できる

### ADR作成のガイドライン

- 新しい機能を提案・実装する前に、ADRに同様の提案が既に却下されていないか確認すること
- ユーザーから明示的に指示された場合にのみ、新しいADRを追加すること
- ADRの作成には `.claude/skills/create-adr` スキルを使用できる（ただし、ユーザーからの明示的指示がある場合のみ）
